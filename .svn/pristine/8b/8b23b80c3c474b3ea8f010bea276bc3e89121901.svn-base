...
class Daftar_Sekolah extends CI_Controller
{
    ...
    function insert_sekolah()
	{
		...
		$data2 = array(
			...
			$kode = $this->input->post('verifi');
      $cek_kode = $this->M_sekolah->cek_kode($kode)->num_rows();
      $email = $this->M_sekolah->cek_emailal($this->input->post('email2'))->num_rows();
		if ($this->input->post('nama2') == null) {
			...
			$result = array('success' => false, 'msg' => 'Harap Isi Email Anda');
		...
		} else if ($this->input->post('verifi') == null) {
			$result = array('success' => false, 'msg' => 'Harap Isi Kode Otentikasi');
		} else if ($email) {
			$result = array('success' => false, 'msg' => 'Email Sudah Terdaftar');
		} else if ($cek_kode == 0) {
			$result = array('success' => false, 'msg' => 'Kode Otentikasi Salah');
		} else {
			$time = $this->M_sekolah->get_time_reset($kode)->result();
			//add 1 hour to time
			foreach($time as $row){
			...
			$result = array('success' => true, 'nuptk' => $data['nuptk'], 'password' => $data['password'], 'email' => $data['email']);
				...
				$result = array('success' => false, 'msg' => 'Waktu Otentikasi Habis');
			}
		}
		echo json_encode($result);
	}
    ...
    {    
         ...
         $mail->setFrom("adminemail@intens.co.id", "Operator Sekolah");
	...
	public function kode_verif()
    {    
		$this->load->helper('string');
		$nik = $this->input->post('nik');
		$reset_key =  random_string('numeric', 4);
		$update = date("Y-m-d H:i:s");
		$time = $this->M_sekolah->get_time_reset($reset_key)->result();
          //add 1 hour to time
          foreach($time as $row){
            $update = $row->xupdate_reset;
          }
		  $expired = date("Y-m-d H:i:s",strtotime('3 minutes',strtotime($update)));
		  
		if($this->M_sekolah->update_kode_otentikasi($nik,$reset_key,$update))
  		{
         $result = new PHPMailer();
         $result->isSMTP();
         $result->Host     = 'mail.intens.co.id';
         $result->Port     = 465;
         $result->SMTPAuth = true;
         $result->Username = "adminemail@intens.co.id";
         $result->Password = "adminemail1234";
         $result->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;      
         $result->isHTML(true);

         $result->setFrom("adminemail@intens.co.id", "Admin Sekolah");
         $result->addAddress($this->input->post('email2'));
		 // Email subject
		 $result->Subject = "Kode Otentikasi Operator Sekolah";
         $result->Body = 'Kode Otentikasi anda : <br><br>
		 <center><h1>'. $reset_key . '</h1></center><br><br>
		 Harap segara mengkonfirmasi kode tersebut. Penggunaan akun diluar ketentuan yang sudah ditetapkan bukan tanggung jawab Tim Pengelola Email Siswa Nasional.
		 <br><br>
		 Regards <br><br><br>
		 Tim Pengelola Email Siswa Nasional';
		 ...
		 // Send email
			if ($this->input->post('nama2') == null) {
				...
				$result->send();
				$result = array('success' => true, 'msg' => 'Silahkan cek email anda untuk melihat kode otentikasi!');
			}
	 	} else {
			$result = array('success' => false, 'msg' => 'Terjadi Kesalahan!');
		 }
		 echo json_encode($result);  
	}  
