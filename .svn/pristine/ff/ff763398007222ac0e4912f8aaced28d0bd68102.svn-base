...
class Ion_auth_model extends CI_Model
{
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select([
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.userid as id',
			    $this->tables['users'].'.userid as user_id'
			]);
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db->join(
				    ...
				    $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.userid',
				    'inner'
				);
			}
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['users'].'.userid', 'desc');
		$this->where($this->tables['users'].'.userid', $id);
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], ['last_login' => time()], ['userid' => $id]);
