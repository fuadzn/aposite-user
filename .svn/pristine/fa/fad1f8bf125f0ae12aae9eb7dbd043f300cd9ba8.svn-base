...
class M_role extends CI_Model{
	...
	function get_all(){
		...
		// 	where is_active = 1
		// 	order by role asc");
		$this->db->select('*')->from('dyn_role')->where('is_active', '1')->order_by('role', 'asc');
		return $this->db->get();
	}
	
	function get_all_withoutadministrator(){
		// return $this->db->query("select *
		// 	from dyn_role
		// 	where is_active = 1 and id<>1
		// 	order by role asc");
		$this->db->select('*')->from('dyn_role')->where('is_active', '1')->where('id <>', '1')->order_by('role', 'asc');
		return $this->db->get();
	}
