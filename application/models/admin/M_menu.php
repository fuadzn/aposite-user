...
class M_menu extends CI_Model{
	...
	function get_child_role($parent_id,$role_id){
		return $this->db->query("SELECT b.page_id,b.is_parent,b.title,IFNULL(a.role_id,0) AS sts FROM dyn_role_menu a RIGHT JOIN dyn_menu b ON a.menu_id=b.page_id AND a.role_id=$role_id WHERE b.show_menu=1 AND b.parent_id=$parent_id ORDER BY b.position ASC");
	}
	...
	function get_page_set($url){
		return $this->db->query("SELECT IFNULL( (SELECT IF (parent_id = 0,page_id,parent_id) FROM dyn_menu WHERE url='$url' LIMIT 1) ,'0') AS page_parent");
	}
